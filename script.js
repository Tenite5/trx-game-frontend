...
        });
        ...
        withdrawButton.addEventListener('click', async () => {
            ...
            if (!tronWalletAddress) {
                alert("TRX wallet address is required.");
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    alert(data.message);
                    fetchBalance(token); // Update the balance display after a successful withdrawal
                } else {
                    alert(`Withdrawal failed: ${data.message}`);
                }
            } catch (error) {
                console.error('Frontend withdrawal error:', error);
